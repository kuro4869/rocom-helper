...
> [!CAUTION]
> Windows 游戏端直接运行辅助工具存在封号风险。只要使用第三方工具就无法承诺绝对安全，请自行评估并承担风险。
>
> **推荐顺序：软路由 Docker SOCKS5 / 抓包 > NAS Docker SOCKS5 > 手机流量经电脑转发 > Windows 游戏机直接运行。**
>
> SOCKS5、手机热点等方案只能让工具与游戏进程分离、降低直接运行带来的风险，不能保证不会封号。

> [!IMPORTANT]
> 请先启动工具和流量解析，再点击游戏中的 `进入世界`。如果进入世界后才启动，请退出到登录界面后重新进入，确保工具能取得完整会话和解密所需信息。

> [!NOTE]
> 本项目只提供 PVE、地图资源采集、宠物筛选等功能，不提供 PVP 或其他影响游戏平衡的功能。
